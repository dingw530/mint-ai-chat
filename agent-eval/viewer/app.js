...
function renderVersionControls(report, versions) {
  ...
  const options = versions.map(version => `<option value="${esc(version.id)}" ${version.id === currentId ? 'selected' : ''}>${esc(version.id)} · ${new Date(version.generatedAt).toLocaleString('zh-CN')}</option>`).join('');
  const baselineOptions = `<option value="">不选择基线</option>${versions.map(version => `<option value="${esc(version.id)}" ${version.id === baselineId ? 'selected' : ''}>${esc(version.id)} · ${new Date(version.generatedAt).toLocaleString('zh-CN')}</option>`).join('')}`;
