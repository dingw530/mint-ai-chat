---
title: "LLM Wiki v2 — 构建个人知识库的模式（增强版）"
created: "eval"
source: "2026-07-24-llm-wiki-v2-构建个人知识库的模式-增强版.md"
---

# LLM Wiki v2 — 构建个人知识库的模式（增强版）

> 原始资料，不可变。摄入日期：2026-07-24



---
## 文件：LLM Wiki v2 — 构建个人知识库的模式（增强版）
...
Your explorations are a source, just like an article or a paper. Every time you solve a problem, you're generating knowledge that should feed back into the system.

## Implementation considerations

From running this at scale, some practical lessons:

**Token budgets are real.** Long sources need chunking. 4K token chunks with 20% overlap work well. Smaller chunks lose context. Larger chunks increase cost with diminishing returns.

**Idempotency matters.** Every ingest operation should be idempotent. If the network fails and you retry, you don't want duplicates. Natural keys: source URL + content hash.

**Latency budget.** LLM calls are slow. The pipeline should be async. Return immediately to the user, queue the rest. The LLM doesn't need to wait for graph updates to continue its session.

**Cost model.** Roughly 70% of your cost goes to ingestion, 30% to queries. This is the opposite of typical RAG (cheap ingestion, expensive queries). Budget accordingly.

**Version compatibility.** Stronger models (GPT-4 class) produce substantially better wiki content. Dropping model quality causes the consolidation pipeline to fail. Prefer stronger models for ingestion, and you can degrade on query side.

## What's next

The pattern is converging on something closer to a living knowledge base than a static wiki. The next frontier is the wiki as an autonomous entity: ingesting, consolidating, and offering knowledge without being asked. Not just answering questions, but proactively identifying gaps in understanding and filling them.

The original LLM Wiki was a file structure. This version is moving toward something that behaves more like a system. The taxonomy in this document organizes features that many implementations have separately but that haven't been treated as parts of a coherent whole. Memory lifecycle, graph traversal, event-driven automation, quality self-correction, multi-agent collaboration, privacy governance, and crystallization are not separate features. They're a single system that learns like a person does: accumulating, consolidating, forgetting, and growing.
